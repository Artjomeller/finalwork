...
class View(Tk):
    def __init__(self, controller, model):
        ...
        self.__width = 630
        self.__height = 350
        self.title("Andmete otsimine")

        self.center(self, self.__width, self.__height)
        self.__frame_top, self.__frame_bottom, self.__frame_bottom2 = self.create_frames()

        (self.__search_lbl, self.__open_file_lbl, self.__filename_lbl, self.__row_count_lbl, self.__count_lbl,
         self.__error_lbl) = self.create_labels()

        self.__search_input = Entry(self.__frame_top, font=self.__default)
        self.__search_input['state'] = 'disabled'
        self.__search_input.grid(row=0, column=2, padx=5, pady=2, sticky=EW)
    ...
    def create_frames(self):
        top = Frame(self, height=50, background="green")
        bottom = Frame(self)
        bottom2 = Frame(self, height=10)

        top.pack(fill=BOTH)
        bottom.pack(expand=YES, fill=BOTH)
        bottom2.pack(expand=NO, fill=BOTH)

        return top, bottom, bottom2

    def create_buttons(self):
        search = Button(self.__frame_top, text='Otsi', font=self.__default, state='disabled',
        ...
        search.grid(row=0, column=3, padx=5, pady=5, sticky=EW)
    ...
    def create_labels(self):
        search_lbl = Label(self.__frame_top, text='Sisesta otsing:', anchor='w', font=self.__default)
        search_lbl.grid(row=0, column=1, padx=5, pady=2, sticky=EW)

        open_file_lbl = Label(self.__frame_bottom2, text='Avatud fail:', anchor='w', font=self.__default)
        open_file_lbl.grid(row=0, column=0, padx=5, pady=2, sticky=EW)
        file_name_lbl = Label(self.__frame_bottom2, text='Fail pole valitud', anchor='w', font=self.__default)
        file_name_lbl.grid(row=0, column=1, padx=5, pady=2, sticky=EW)

        row_count_lbl = Label(self.__frame_bottom2, text='Ridu kokku:', anchor='w', font=self.__default)
        row_count_lbl.grid(row=0, column=2, padx=5, pady=2, sticky=EW)

        count_lbl = Label(self.__frame_bottom2, text='0', anchor='w', font=self.__default)
        count_lbl.grid(row=0, column=3, padx=5, pady=2, sticky=EW)

        error_lbl = Label(self.__frame_top, text='', anchor='w', font=self.__default, bg="green", fg="red")
